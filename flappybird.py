...
def center(toplevel):
    ...
    print("toplevel.geometry is " + str(toplevel.geometry()))
    print("toplevel.geometry().split('+')[0] " + str(toplevel.geometry().split('+')[0]))
    print("toplevel.geometry().split('+')[0].split('x') " + str(toplevel.geometry().split('+')[0].split('x')))
    size = tuple(int(_) for _ in toplevel.geometry().split('+')[0].split('x'))
    x = w / 2 - size[0] / 2
    print("x is " + str(x))
    y = h / 2 - size[1] / 2 - 35
    print("y is " + str(y))
    print("toplevel.geometry('%dx%d+%d+%d' % (size + (x, y))) " + str(toplevel.geometry("%dx%d+%d+%d" % (size + (x, y)))))
...
birdImg = PhotoImage(file="/Users/acvanderkolk/PycharmProjects/GameOfLifeTwo/images/bird.gif")
